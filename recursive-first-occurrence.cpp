...
int recursive_first_occurrence(int a[],int num,int low,int high,int first_index){
	if(low>high)
		return first_index;
	int mid = (low+high)/2;
	if(num == a[mid])
			return recursive_first_occurrence(a,num,low,mid-1,mid);
	if(num > a[mid])
			return recursive_first_occurrence(a,num,mid+1,high,first_index);
	return recursive_first_occurrence(a,num,low,mid-1,first_index);
}
...
int main()
{
	...
	int no;
	cin>>no;
	cout<<recursive_first_occurrence(a,no,0,n-1,-1)<<endl;
	return 0;
}